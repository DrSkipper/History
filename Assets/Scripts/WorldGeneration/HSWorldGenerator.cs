...
public class HSWorldGenerator : WorldGenerator
{
    ...
    private IEnumerator<RunStageDelegate> GetStages()
    {
        while (_currentStage < _stageDelegates.Length)
        {
            if (!_stagesCompleted[_currentStage])
                yield return _stageDelegates[_currentStage];
            else
                ++_currentStage;
        }
    }
    ...
    {
        if (_stages.MoveNext())
            _stages.Current(_currentStage, frames);
        else
    ...
    public override void clearMap()
    {
        ...
        _stagesCompleted = new bool[_stageDelegates.Length];
        _framesRunByStage = new int[_stageDelegates.Length];
        _currentStage = 0;
    }
    ...
    private IEnumerator<RunStageDelegate> _stages;
